function __init__() {
    let tabledata = createParentTabledata(); //tabledata作成
    creatEelement(tabledata, "table"); //DOM作成
    innsertnowData();
    console.log("Init_done");
}

function creatEelement(tabledata, classname) {
    /**
     * DOM操作　table以下要素の作成
...
function createParentTabledata() {
    /**
     * tableのデータを作成
     * @　return=tabledata object 2次元配列
     */
    let pr = openSystemdata(); //プライマリー読込
    ...
    for (let i = 0; i < parentlen; i++) {
        ...
        pdatetime.push(`約${Math.floor(keikacalc(pk)/30)}ヶ月<br>(${keikacalc(pk)}日)`);
        //console.log(kobj);
        pkeika.push(kobj);
...
//let btn = document.getElementById('updatetable')
